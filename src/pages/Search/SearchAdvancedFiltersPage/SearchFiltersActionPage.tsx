...
function SearchFiltersActionPage() {
    ...
    const listData: Array<ListItem<SearchAction>> = useMemo(() => {
        return getActionOptions(translate).map((actionOption) => ({
            text: actionOption.text,
            keyForList: actionOption.value,
            isSelected: selectedItem === actionOption.value,
        }));
    }, [translate, selectedItem]);

    const updateSelectedItem = useCallback((item: ListItem<SearchAction>) => {
        setSelectedItem(item?.keyForList ?? undefined);
    }, []);
    ...
    return (
        <ScreenWrapper
            testID="SearchFiltersActionPage"
