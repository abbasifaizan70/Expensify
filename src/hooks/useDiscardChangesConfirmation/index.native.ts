    ...
    const showDiscardModal = (blockedAction?: NavigationAction) => {
        ...
        KeyboardUtils.dismiss().then(() => {
            showConfirmModal({
                ...
                // Opt this modal out of focus restoration. On hide, BaseModal calls
                // `ComposerFocusManager.refocusAfterModalFullyClosed`, which resolves asynchronously and therefore
                // lands AFTER a confirmed discard has already navigated away. `restoreFocusState` then falls through
                // to `tryRestoreTopmostFocus` and focuses the composer on the screen underneath, re-opening the
                // keyboard we just dismissed: the composer jumps up and the next tap is spent blurring it instead of
                // pressing the row that was tapped (#97127). Cancelling needs no restoration either, because
                // consumers refocus their own input from `onCancel`.
