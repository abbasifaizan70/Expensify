...
### [@shopify+flash-list+2.3.0+017+fix-autoscroll-to-bottom-overriding-scroll-to-index.patch](@shopify+flash-list+2.3.0+017+fix-autoscroll-to-bottom-overriding-scroll-to-index.patch)

- Reason: Stops FlashList's `maintainVisibleContentPosition` autoscroll-to-bottom from overriding a programmatic `scrollToIndex`. `useBoundDetection.checkBounds` latches `pendingAutoscrollToBottom` whenever the viewport sits at the physical end of the content, and `runAutoScrollToBottomCheck` consumes that latch on the next `data` / content-size change by queueing `scrollViewRef.scrollToEnd()` in a `requestAnimationFrame` — without asking whether an explicit `scrollToIndex` is already driving the offset. For the App's inverted chat lists the physical end is the *oldest*-message side (the visual top), so when a user scrolls up to the top of an expense report and sends a message, the App's `scrollToIndex({index: 0})` lands on the new message and FlashList's queued `scrollToEnd()` then drags the list straight back to the top (measured on web: the App scroll lands at offset 0, and ~120ms later `useBoundDetection` → `scrollToEnd` moves it to the content end). This is the race Shopify fixed upstream in [PR #2290](https://github.com/Shopify/flash-list/pull/2290) (released in 2.3.2): `runAutoScrollToBottomCheck` returns early while `recyclerViewManager.isOffsetProjectionEnabled` is `false`, which `scrollToIndex` sets for its whole lifetime and restores once the scroll settles. Backported here on top of the 2.3.0 patch stack, with two small additions: the same check is repeated inside the queued animation frame (a `scrollToIndex` can start between the effect scheduling the frame and the frame firing), and the `scrollToIndex` "index out of bounds → scrollToEnd" exit now re-enables offset projection and offset correction before delegating, since that exit otherwise returns with projection left disabled and would keep autoscroll-to-bottom suppressed for the rest of the list's life. Normal bottom-stick (no programmatic scroll in flight) is unchanged.
- Files changed: `src/recyclerview/hooks/useBoundDetection.ts`, `src/recyclerview/hooks/useRecyclerViewController.tsx`, and their `dist` counterparts (`dist/recyclerview/hooks/useBoundDetection.js`, `dist/recyclerview/hooks/useRecyclerViewController.js`).
- Upstream PR/issue: https://github.com/Shopify/flash-list/pull/2290 (fixed in 2.3.2 — drop this patch when upgrading to >= 2.3.2)
- E/App issue: https://github.com/Expensify/App/issues/99559
